...
public class GPanColor extends JPanel implements MouseListener {
    ...
    public GPanColor(GFrame frame) {
        ...
        var nodeiter = GFrame.graph.nodeIter();
        ...
        var edgeIter = GFrame.graph.edgeIter();
        while (edgeIter.hasNext()) {
            var edge = edgeIter.next();

            var x1 = Scale.scale(GFrame.graph.getNode(edge.getSrc()).getLocation().x(), 35.177, 35.23, 0, width - 150);
            var x2 = Scale.scale(GFrame.graph.getNode(edge.getDest()).getLocation().x(), 35.177, 35.23, 0, width - 150);
            var y1 = Scale.scale(GFrame.graph.getNode(edge.getSrc()).getLocation().y(), 32.101, 32.2, 150, height * 7);
            var y2 = Scale.scale(GFrame.graph.getNode(edge.getDest()).getLocation().y(), 32.101, 32.2, 150, height * 7);
